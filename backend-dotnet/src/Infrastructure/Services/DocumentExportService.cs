...
{
    public string BuildRtf(DocumentDetailDto detail)
    {
        var sb = new StringBuilder();
        sb.Append("{\\rtf1\\ansi\\ansicpg1252\\uc1\\deff0\n");
        sb.Append("\\b SISTEMA DE LECTURA INTELIGENTE \\b0\\par\n");
        sb.Append($"Documento: {EscapeRtf(detail.OriginalFilename)}\\par\n");
        sb.Append($"Tipo: {EscapeRtf(detail.DocumentType.ToString())}\\par\n");
        sb.Append($"Fecha de carga: {detail.UploadedAt:yyyy-MM-dd HH:mm}\\par\n");
        sb.Append("\\par\\b Campos extraidos \\b0\\par\n");

        var (exportFields, fieldMap, derivedValues) = PrepareExportFields(detail, useAccentedLabels: true);
        ...
        foreach (var exportField in exportFields)
        {
            ...
            {
                var value = EscapeRtf(GetDisplayFieldValue(key, field.CorrectedValue ?? field.Value));
                sb.Append($"\\b {EscapeRtf(label)}: \\b0 {value}\\par\n");
            }
            else if (derivedValues.TryGetValue(key, out var derived))
            {
                var value = EscapeRtf(GetDisplayFieldValue(key, derived));
                sb.Append($"\\b {EscapeRtf(label)}: \\b0 {value}\\par\n");
            }
            else
            {
                sb.Append($"\\b {EscapeRtf(label)}: \\b0 -\\par\n");
            }
        }
        ...
        if (structuredTables.Count > 0)
        {
            ...
            foreach (var table in structuredTables)
            {
                ...
                sb.Append($"\\b {EscapeRtf(tableTitle)} \\b0\\par\n");
                AppendRtfTable(sb, table.Rows);
            }
        }
    ...
    private static (
        List<(string Key, string Label)> ExportFields,
        Dictionary<string, DocumentFieldDto> FieldMap,
        IReadOnlyDictionary<string, string> DerivedValues
    ) PrepareExportFields(DocumentDetailDto detail, bool useAccentedLabels)
    {
        var template = GetTemplate(detail.DocumentType, useAccentedLabels);
        ...
        return (exportFields, fieldMap, derivedValues);
    }

    private static IReadOnlyList<(string Key, string Label)> GetTemplate(DocumentType documentType, bool accented)
    {
        return documentType switch
        {
            DocumentType.Ine => new List<(string, string)>
            {
                ...
                ("sexo", "Sexo"),
                ("domicilio", "Domicilio"),
                ("seccion", accented ? "Sección" : "Seccion"),
                ("vigencia", "Vigencia")
            },
            ...
            DocumentType.ActaNacimiento => new List<(string, string)>
            {
                ...
                ("folio", "Folio"),
                ("numero_acta", accented ? "Número de acta" : "Numero de acta"),
            ...
            {
                ("proveedor", "Proveedor"),
                ("numero_servicio", accented ? "Número de servicio" : "Numero de servicio"),
                ("cuenta", "Cuenta"),
                ("referencia", "Referencia"),
                ("titular", "Titular"),
                ("domicilio", "Domicilio"),
                ("cp", accented ? "Código postal" : "Codigo postal"),
                ("fecha_limite", accented ? "Fecha límite" : "Fecha limite"),
                ("total", "Total")
            },
        ...
        IReadOnlyList<IReadOnlyList<string>> rows)
    {
        ...
        for (var rowIndex = 0; rowIndex < rows.Count; rowIndex++)
        {
            ...
            for (var colIndex = 0; colIndex < columnCount; colIndex++)
            {
                ...
                if (rowIndex == 0)
                {
                    target.Append("\\intbl\\b ").Append(EscapeRtf(value)).Append("\\b0\\cell");
                }
                else
                {
                    target.Append("\\intbl ").Append(EscapeRtf(value)).Append("\\cell");
                }
            }
            target.Append("\\row\n");
        }
    ...
    private static Cell TextCell(string cellRef, string? value)
    {
        return new Cell
        {
    ...
    internal sealed record StructuredExportTable(int Index, string? Source, IReadOnlyList<IReadOnlyList<string>> Rows);
    ...
    internal static string GetDisplayFieldValue(string key, string? value)
    {
