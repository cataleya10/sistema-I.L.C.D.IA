...
class TestDigitNormalization:
    ...
    def test_nss_normalizes_letters_to_digits(self):
        fields = [{"key": "nss", "value": "1234O67890I"}]
        result = _run(validate_fields(fields))
        assert result[0]["value"] == "12340678901"

    def test_clabe_normalizes_letters_to_digits(self):
        fields = [{"key": "clabe", "value": "OI234567890L234567"}]
        result = _run(validate_fields(fields))
        assert result[0]["value"] == "012345678901234567"

    def test_cp_normalizes_letters(self):
        fields = [{"key": "cp", "value": "O6O0O"}]
        result = _run(validate_fields(fields))
        assert result[0]["value"] == "06000"

    def test_seccion_normalizes_letters(self):
...
class TestEdgeCases:
    ...
    def test_empty_list(self):
        result = _run(validate_fields([]))
        assert result == []

    def test_normalization_does_not_affect_non_numeric_fields(self):
