  ...
  template: `
    <section class="page" *ngIf="document; else loading">
      ...
      <div class="grid" [class.grid--full]="hideExtractedResults">
        ...
        <div class="fields" *ngIf="!hideExtractedResults">
          ...
          <table class="results" *ngIf="displayFields.length">
            ...
            <tbody>
              <tr *ngFor="let field of displayFields; trackBy: trackByFieldKey">
                <td>{{ field.label }}</td>
                <td>
      ...
      <details class="logs">
        ...
        <div class="log" *ngFor="let log of logs; trackBy: trackByIndex">
...
export class DocumentsDetailPage implements OnInit, OnDestroy {
  ...
  private isHiddenField(key: string | null | undefined, documentType?: string | null): boolean {
    ...
    if (DocumentsDetailPage.HIDDEN_FIELD_KEYS.has(normalizedKey)) {
      return true;
    }
    if (normalizedKey.startsWith('tabla_celdas')) {
      const normalizedType = String(documentType ?? '').toUpperCase();
      return !DocumentsDetailPage.TABLE_VISIBLE_TYPES.has(normalizedType);
    }
    return false;
  }
  ...
  get textoDetectadoField(): DocumentDetail['fields'][number] | undefined {
